    ...
    (async () => {
      try {
        const { data } = await axios.get(
          `https://nestinn.cyclic.cloud/api/hotels?category=${hotelCategory}`
        );
    ...
    <div className="relative">
      ...
      {hotels && hotels.length > 0 ? (
        <InfiniteScroll
          ...
          hasMore={hasMore}
          loader={
            hotels.length > 0 && <h3 className="alert-text">...</h3>
          }
          endMessage={<p className="alert-text">Your nest is waiting to be discovered! Explore and make it yours :D</p>}
        >
